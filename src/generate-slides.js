...
const CONFIG_PATH = resolveProjectPath(process.argv[2] || './content/templates/ete-transformation.json')
const OUTPUT_DIR = resolveProjectPath(process.argv[3] || './output')
const W = 1080
const H = 1920
...
for (const f of FONTS) {
  const fontPath = resolveProjectPath(f.path)
...
function pickRandomFromTheme(themeName, wantedTags = []) {
  const candidates = [
    resolveProjectPath(`./pinterest_images/${themeName}`),
    resolveProjectPath(`./avatars/${themeName}`),
  ]
...
async function generateSlide(slide, index) {
  ...
  const imagePath = slide.imagePath ? resolveProjectPath(slide.imagePath) : null
